import { i as TSS_SERVER_FUNCTION, r as createServerFn } from "./server-CJcie1Pg.mjs";
import { t as requireSupabaseAuth } from "./auth-middleware-CgDK2GLf.mjs";
import { i as string, n as array, r as object, t as number } from "../_libs/zod.mjs";
//#region node_modules/.nitro/vite/services/ssr/assets/receipt.functions-DTVdtgE1.js
...
var receiptSchema = object({
	supplier: string().nullish().transform((v) => v ?? null),
	reference: string().nullish().transform((v) => v ?? null),
	order_date: string().nullish().transform((v) => v ?? null),
	shipping_cost: number().nullish().transform((v) => v ?? null),
	notes: string().nullish().transform((v) => v ?? null),
	items: array(itemSchema).default([])
});
...
var scanReceipt = createServerFn({ method: "POST" }).middleware([requireSupabaseAuth]).validator((input) => object({ dataUrl: string().min(32) }).parse(input)).handler(scanReceipt_createServerFn_handler, async ({ data, context }) => {
